...
public class FacilityGenericTester {
	...
	@Test
 	...
 	public void testSetPhysician() {
 		phase3Facility.setPhysician(new UHealthID("XXXX-1111"), 11111);
 		assertEquals(11111, (int) phase3Facility.lookupByPhysician(11111).get(0).getPhysician());
 	}
 	
 	@Test
	public void testSetDateSmall() {
 		phase3Facility.setLastVisit(new UHealthID("XXXX-1111"), new GregorianCalendar(2024, 1, 11));

		assertEquals(new GregorianCalendar(2024, 1, 11), phase3Facility.lookupByPhysician(7).get(0).getLastVisit());
	}
 	
 	@Test
 	public void testRecentPats() {
 		ArrayList<CurrentPatientGeneric<Integer>> patsCutoff = phase3Facility.getRecentPatients(new GregorianCalendar(2019, 1, 3));
 		ArrayList<CurrentPatientGeneric<Integer>> patsNew = new ArrayList<CurrentPatientGeneric<Integer>>();
 		patsNew.add(new CurrentPatientGeneric<Integer>("R", "T", new UHealthID("BBBB-1111"), 7, new GregorianCalendar(2019, 1, 3)));
 		assertEquals(patsCutoff.get(0), patsNew.get(0));
	}
